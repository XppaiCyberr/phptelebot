...
function messageCard(m,depth){const bot=m.direction==='out';return `<article class="rounded-xl border ${bot?'border-cyan-400/25 bg-cyan-400/10':'border-white/10 bg-slate-900/70'} p-3 transition hover:bg-slate-800"><div class="mb-1.5 flex flex-wrap items-center gap-1.5 text-[11px] text-slate-400"><span>${bot?'Bot reply':'User'}</span><span>${time(m.created_at)}</span><span>${esc(m.update_type)}/${esc(m.message_type)}</span><span>chat ${esc(m.chat_id)}</span>${m.topic_id?`<span>topic ${esc(m.topic_name||('ID '+m.topic_id))}</span>`:''}${m.message_id?`<span>msg ${esc(m.message_id)}</span>`:''}</div><div class="text-xs font-medium ${bot?'text-cyan-100':'text-cyan-200'}">${bot?'Bot':esc([m.first_name,m.last_name].filter(Boolean).join(' ')||m.username||m.user_id||'Unknown')}</div><p class="mt-1.5 whitespace-pre-wrap text-sm leading-5">${esc(m.text||'[non-text update]')}</p><button class="mt-2 rounded-lg bg-white/10 px-2.5 py-1 text-[11px] transition hover:bg-cyan-400 hover:text-slate-950" onclick="fillReply('${esc(m.chat_id)}','${esc(m.topic_id||'')}','${esc(m.message_id||'')}')">Reply</button></article>${(m.children||[]).map(c=>`<div class="ml-4 border-l border-cyan-400/30 pl-3">${messageCard(c,depth+1)}</div>`).join('')}`}
function renderMessages(messages){const map={},roots=[];messages.slice().reverse().forEach(m=>{m.children=[];map[m.chat_id+':'+m.message_id]=m});messages.slice().reverse().forEach(m=>{const key=m.chat_id+':'+m.reply_to_message_id;if(m.reply_to_message_id&&map[key])map[key].children.push(m);else roots.push(m)});setHtml('messages',roots.reverse().map(m=>messageCard(m,0)).join('')||'<p class="text-sm text-slate-400">No messages logged yet.</p>')}
